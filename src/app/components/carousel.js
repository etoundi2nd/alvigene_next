...
export default function Carousel({ url, name, comment, city }) {
    useEffect(() => {
        ...
        function handleMobileMenu() {
            var menuTrigger = document.querySelector('[data-action="click->marketing--mobile#toggleMenu"]')
            var mobileMenu = document.querySelector('[data-marketing--mobile-target="menu"]')
    ...
    return (
        <div className="main-customer d-none" id="main-customer-0" data-slide="slide">
            <div className="message py-1">{comment}</div>
            <div className="customer-details py-2">
                <div className="mb-1">
                    <Image width={300} height={300} className="avatar" src={url} alt="rose-image" />
                </div>
                <h5>{name}</h5>
